...
trait MonitorStatusDB {
  ...
  val defaultStatus: List[MonitorStatus] = List(
    ...
    MonitorStatus(CalibratedStat, "空白", InternalPriorityMax),
    MonitorStatus(CalibrationSampleStat, "標準品", InternalPriorityMax),
    ...
    MonitorStatus(LessThanMDL, "<MDL", InternalPriorityMax)
  )
