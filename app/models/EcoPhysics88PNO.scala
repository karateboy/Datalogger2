...
object EcoPhysics88PNO extends AbstractDrv(_id = "EcoPhysics88P_NO", name = "Eco Physics 88P-NO Only",
  ...
  override def factory(id: String, protocol: ProtocolParam, param: String)(f: AnyRef, fOpt: Option[AnyRef]): Actor = {
    val f2 = f.asInstanceOf[EcoPhysics88PNO.Factory]
    val config = Json.parse(param).validate[DeviceConfig].asOpt.get
...
class EcoPhysics88PNOCollector @Inject()(instrumentOp: InstrumentDB, monitorStatusOp: MonitorStatusDB,
  ...
  logger.info(s"EcoPhysics88P NO only collector start")
  ...
  override def probeInstrumentStatusType: Seq[InstrumentStatusType] =
    EcoPhysics88PNO.instrumentStatusKeyList
      ...
      try {
        val ret =
          for (serial <- serialOpt) yield {
            ...
            if (reply != null) {
              val tokens = getDataPart(reply)
              val regs = List((EcoPhysics88PNO.instrumentStatusKeyList(0), tokens(0).toDouble))
  ...
  override def getDataRegList(deviceConfig: DeviceConfig): Seq[DataReg] = EcoPhysics88PNO.getDataRegList(deviceConfig)
