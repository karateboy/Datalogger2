...
class CalibrationOp @Inject()(mongodb: MongoDB) extends CalibrationDB {
  ...
  lazy val codecRegistry: CodecRegistry = fromRegistries(fromProviders(classOf[Calibration]), DEFAULT_CODEC_REGISTRY)
  ...
  collection.createIndex(Indexes.descending("monitor", "startTime"), new IndexOptions().unique(false))
  collection.createIndex(Indexes.descending("monitor", "monitorType", "startTime"), new IndexOptions().unique(false))

  import org.mongodb.scala._
  import org.mongodb.scala.model.Indexes._

  override def calibrationReport(start: DateTime, end: DateTime)(monitor:String): Seq[Calibration] = {
  ...
  override def calibrationReportFuture(start: DateTime, end: DateTime)(monitor:String): Future[Seq[Calibration]] = {
    import org.mongodb.scala.model.Filters._
    import org.mongodb.scala.model.Sorts._
  ...
  override def calibrationReportFuture(start: DateTime)(monitor:String): Future[Seq[Calibration]] = {
    import org.mongodb.scala.model.Filters._
    import org.mongodb.scala.model.Sorts._
    val f = collection.find(and(
      gte("startTime", start.toDate),
  ...
  override def calibrationReport(mt: String, start: DateTime, end: DateTime)(monitor:String): Seq[Calibration] = {
    import org.mongodb.scala.model.Filters._
    import org.mongodb.scala.model.Sorts._
