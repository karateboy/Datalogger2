...
import akka.io._
import models.ModelHelper.getNextTime
import models.MonitorType.{CH4, NMHC, NO, NO2, NOX, THC}
...
(@Assisted id: String, @Assisted protocol: ProtocolParam, @Assisted config: HoribaConfig) extends Actor {
  ...
  logger.info(s"HoribaApnaCollector created $id:$protocol $config")
  ...
  private def processResponse(data: ByteString)(implicit calibrateRecordStart: Boolean): Unit = {
    ...
    cmd match {
      case "R001" =>
        ...
        val noValue = result(2).substring(5).toDouble
        val no2Value = result(3).substring(5).toDouble
        val noxValue = result(4).substring(5).toDouble
        val data = List(MonitorTypeData(NO, noValue, collectorState),
          MonitorTypeData(NO2, no2Value, collectorState),
          MonitorTypeData(NOX, noxValue, collectorState))
