      ...
      for (resultOptList <- f) {
        ...
        if (lastModifiedList.nonEmpty) {
          ...
          for (current <- getPeriods(start, end, Period.hours(1)))
            dataCollectManagerOp.recalculateHourData(Monitor.SELF_ID, current, true)(monitorTypeOp.mtvList)
        }
      }
      ...
      for (record <- minAvg) yield
        try {
          ...
          Some(RecordList(time = Date.from(dt), monitor = Monitor.SELF_ID,
            mtDataList = Seq(MtRecord(mtName, value, MonitorStatus.NormalStat))))
        } catch {
          case ex: Throwable =>
            None
        }
