...
trait MonitorTypeDB {
  ...
  val defaultMonitorTypes: List[MonitorType] = List(
    ...
    rangeType(LEQA, "LeqA", "dB", 2, acoustic = true),
    rangeType(LEQZ, "LeqZ", "dB", 2, acoustic = true),
    rangeType(LDN, "LDN", "dB", 2, acoustic = true),
    rangeType(LFN, "LFN", "dB", 2, acoustic = true),
  ...
  def addMeasuring(mt: String, instrumentId: String, recordDB: RecordDB): Unit = {
    val newMtCase =
      synchronized {
        if (!map.contains(mt)) {
          val mtCase = defaultMonitorTypes.find(_._id == mt)
            .getOrElse({
              if (mt.endsWith("_TEST")) {
                val srcMt = defaultMonitorTypes.find(_._id == mt.reverse.drop("_TEST".length).reverse)
                if (srcMt.isDefined) {
                  val srcMtCase = srcMt.get
                  srcMtCase.copy(_id = mt, desp = srcMtCase.desp + "(TEST)")
                } else
                  rangeType(mt, mt, "??", 2)
              } else
                rangeType(mt, mt, "??", 2)
            })
          mtCase.addMeasuring(instrumentId, append = false)
          upsertMonitorType(mtCase)
          mtCase
        } else {
          val mtCase = map(mt)
