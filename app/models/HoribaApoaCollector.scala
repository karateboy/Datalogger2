...
(@Assisted id: String, @Assisted protocol: ProtocolParam, @Assisted config: HoribaConfig) extends Actor {
  ...
  import HoribaApoaCollector._
  import TapiTxx._

  import scala.concurrent.duration._
  import scala.concurrent.{Future, blocking}

  logger.info(s"HoribaApoaCollector created $id:$protocol $config")
  ...
  private def processResponse(data: ByteString)(implicit calibrateRecordStart: Boolean): Unit = {
    ...
    cmd match {
      case "R001" =>
        ...
        val o3Value = result(2).substring(5).toDouble
        val data = List(MonitorTypeData(MonitorType.O3, o3Value, collectorState))
